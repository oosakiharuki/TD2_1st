...
Sound::Sound(const std::wstring& filePath) : hMmio(nullptr), waveData(nullptr), pXAudio2(nullptr), pMasteringVoice(nullptr), pSourceVoice(nullptr) {
	LoadWav(filePath);
...
void Sound::Play() {
	...
	// ソースボイスのボリュームを再設定
	pSourceVoice->SetVolume(currentVolume); // ここで音量を再設定
...
void Sound::SetVolume(float volume) {
	...
	currentVolume = volume; // 現在の音量を保持

	if (pSourceVoice) {
		pSourceVoice->SetVolume(volume); // 音量を設定
	}
}